                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        <div class="text-center mb-4">
                            <div class="inline-block p-1 rounded-circle border mb-3 mx-auto"
                                style="width: 100px; height: 100px;">
                                <i class="bi bi-person-bounding-box text-gray-300"
                                    style="font-size: 80px; line-height: 1;"></i>
                            </div>
                        ...
                        <ul class="list-group list-group-flush small">
                            ...
                            <li class="list-group-item d-flex justify-content-between">
                                ...
                                <span
                                    class="badge badge-pill badge-info px-2">{{ ucfirst($applicant->application_status) }}</span>
                            </li>
                        </ul>
                        ...
                        @else
                            <form action="{{ route('hr1.assessment.performance.validate', $applicant->id) }}" method="POST">
                                ...
                                <button type="submit" class="btn btn-primary btn-sm shadow-sm"
                                    onclick="return confirm('Validate this assessment score?')">
                                            ...
                                            <td class="text-center">
                                                <div class="progress mb-1" style="height: 10px; min-width: 100px;">
                                                    <div class="progress-bar {{ $score->rating >= 75 ? 'bg-success' : 'bg-warning' }}"
                                                        role="progressbar" style="width: {{ $score->rating }}%"></div>
                                                </div>
                                                <span
                                                    class="font-weight-bold {{ $score->rating >= 75 ? 'text-success' : 'text-warning' }}">
                                                    {{ $score->rating }}%
                                                </span>
                                            </td>
                                            <td>
                                                <small
                                                    class="text-muted italic">{{ $score->remarks ?? 'No remarks provided.' }}</small>
                                            </td>
                                            <td>
                                                <small
                                                    class="font-weight-bold text-dark">{{ $score->assessor_name ?? $score->assessed_by ?? 'N/A' }}</small>
                                            </td>
                                            <td>
                                                <small
                                                    class="font-weight-bold text-dark">{{ $score->validator_name ?? $score->validated_by ?? 'N/A' }}</small>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="3" class="text-center text-muted">No specific competency ratings
                                                available.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
