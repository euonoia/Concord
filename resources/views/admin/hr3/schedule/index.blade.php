...
<table border="1" width="100%" style="border-collapse: collapse;">
    <thead>
        <tr style="background-color: #f2f2f2;">
            ...
            <th>Time</th>
            <th>Location</th>
            <th>Validated By</th> {{-- Added Header --}}
        </tr>
    </thead>
    <tbody>
        @forelse($schedules as $schedule)
            <tr>
                ...
                <td>
                    {{-- Displays Employee Name via the relationship --}}
                    {{ $schedule->validator->first_name ?? 'Unknown' }} {{ $schedule->validator->last_name ?? '' }}
                </td>
            </tr>
        @empty
            <tr>
                <td colspan="5" style="text-align: center;">No interviews scheduled yet.</td>
            </tr>
        @endforelse
    </tbody>
</table>
...
<script>
    ...
    document.getElementById('specialization').addEventListener('change', function() {
        ...
        .then(data => {
            ...
            if (data.length === 0) {
                appSelect.innerHTML = '<option value="">No applicants found</option>';
            } else {
                data.forEach(app => {
                    let opt = document.createElement('option');
                    opt.value = app.id;
