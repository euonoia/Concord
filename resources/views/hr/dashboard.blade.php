    ...
    <div id="toast-container" style="position: fixed; top: 25px; right: 25px; z-index: 9999; animation: slideIn 0.4s cubic-bezier(0.68, -0.55, 0.265, 1.55);">
        <div style="background: #ffffff; border-right: 4px solid #ef4444; padding: 1rem 1.5rem; border-radius: 12px; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1); display: flex; align-items: center; gap: 15px; min-width: 320px; border: 1px solid #fee2e2;">
            ...
            <div style="flex-grow: 1;">
                <h4 style="margin: 0; color: #1e293b; font-size: 0.95rem; font-weight: 700;">Action Required</h4>
...
<div class="dashboard-wrapper" style="padding: 2.5rem 1.5rem; max-width: 1300px; margin: 0 auto; font-family: 'Inter', sans-serif; background-color: #f8fafc;">
    ...
    <div class="header-section" style="margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: center;">        
        ...
        <div style="display: flex; align-items: center; gap: 1rem;">
            ...
            <form action="{{ route('attendance.verify') }}" method="POST" style="margin: 0;">
                @csrf
                {{-- Token matches the bypass logic in your UserAttendanceController --}}
                <input type="hidden" name="token" value="DASHBOARD_BUTTON"> 
                
                @if($isClockedIn)
                    <button type="submit" 
                        title="Clock Out Now"
                        style="display: flex; align-items: center; justify-content: center; background: #dc2626; color: white; width: 52px; height: 52px; border-radius: 12px; border: none; cursor: pointer; transition: all 0.2s; box-shadow: 0 4px 12px rgba(220,38,38,0.2);"
                ...
                @else
                    <button type="submit" 
                        title="Clock In Now"
                        style="display: flex; align-items: center; justify-content: center; background: #2563eb; color: white; width: 52px; height: 52px; border-radius: 12px; border: none; cursor: pointer; transition: all 0.2s; box-shadow: 0 4px 12px rgba(37,99,235,0.2);"
            ...
            <div class="date-chip" style="background: #ffffff; padding: 0.8rem 1.2rem; border-radius: 12px; border: 1px solid #e2e8f0; color: #64748b; font-size: 0.875rem; font-weight: 600; box-shadow: 0 1px 2px rgba(0,0,0,0.05); height: 52px; display: flex; align-items: center; gap: 12px;">
    ...
    <div class="dashboard_grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 1.5rem; align-content: start;">            
        ...
        <div class="quick-actions-container" style="background: white; border-radius: 16px; border: 1px solid #e2e8f0; padding: 1.5rem; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.05);">
            ...
            <div style="display: flex; flex-direction: column; gap: 0.75rem; max-height: 400px; overflow-y: auto;">
                @forelse($allShifts as $shift)
                    @php $isToday = $shift->day_of_week == now()->format('l'); @endphp
                    <div style="background: {{ $isToday ? 'linear-gradient(135deg, #2B5F6B 0%, #1e424a 100%)' : '#ffffff' }}; 
                        ...
                        <div style="display: flex; align-items: center; gap: 12px;">
                            <div style="background: {{ $isToday ? 'rgba(255,255,255,0.15)' : '#f1f5f9' }}; width: 40px; height: 40px; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                                <i class="fas {{ $shift->shift_name == 'Morning Shift' ? 'fa-sun' : 'fa-moon' }}" style="color: {{ $isToday ? '#fbbf24' : '#f59e0b' }};"></i>
                            </div>
                            <div>
                                <div style="font-size: 0.85rem; font-weight: 700;">{{ $shift->day_of_week }} @if($isToday) <span style="font-size: 0.65rem; background: #4ade80; color: #064e3b; padding: 2px 6px; border-radius: 4px; margin-left: 5px;">TODAY</span> @endif</div>
                                <div style="font-size: 0.75rem; opacity: 0.8;">{{ $shift->shift_name }} • {{ \Carbon\Carbon::parse($shift->start_time)->format('h:i A') }} - {{ \Carbon\Carbon::parse($shift->end_time)->format('h:i A') }}</div>
                            </div>
                        </div>
                    </div>
                @empty
