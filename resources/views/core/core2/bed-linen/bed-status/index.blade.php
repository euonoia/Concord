...
@section('content')
    <div class="flex justify-between items-start mb-8">
        ...
        <a href="{{ route('core2.bed-linen.bed-status.create') }}"
            class="bg-indigo-600 text-white px-7 py-4 rounded-2xl font-black text-xs uppercase flex items-center gap-3 shadow-lg hover:bg-indigo-700 transition">
    ...
    <div class="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden flex flex-col"
        ...
        <div class="shrink-0 border-t border-slate-200 bg-white p-4">
            ...
            <p class="text-xs text-slate-400 font-medium italic">Click on a bed to update its status (excluding Active
                Admissions).</p>
        </div>
    </div>
    ...
    <div id="updateStatusModal"
        class="fixed inset-0 z-[9999] hidden items-center justify-center flex bg-black/50 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-2xl w-[400px] shadow-2xl overflow-hidden transform scale-95 transition-transform"
            id="updateStatusModalInner">
            ...
            <div class="p-6">
                ...
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-widest mb-3">Select New
                    Status</label>
                <div class="grid grid-cols-2 gap-3" id="statusOptionsGrid">
                    <!-- Status options dynamically generated -->
                </div>
                <div id="updateModalError"
                    class="hidden mt-4 p-3 bg-red-50 text-red-700 rounded-xl text-xs font-semibold border border-red-200">
                    <!-- Error text -->
                </div>
            </div>
            ...
            <div class="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                <button type="button" onclick="closeUpdateModal()"
                    class="px-5 py-2.5 rounded-xl text-xs font-bold text-slate-600 bg-white border border-slate-200 shadow-sm hover:bg-slate-50 transition">Cancel</button>
                <button type="button" onclick="saveBedStatus()" id="saveStatusBtn"
                    class="px-5 py-2.5 rounded-xl text-xs font-bold text-white bg-indigo-600 shadow-sm hover:bg-indigo-700 transition flex items-center gap-2">
                    Save Changes
                </button>
            </div>
        </div>
...
@push('scripts')
    <style>
        ...
        .bp-bed-wrap:hover .bp-bed:not(.bp-bed-occupied) {
    ...
    <script>
        ...
        const bpZones = {
            ...
            'ER': {
                icon: '<svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" clip-rule="evenodd" /></svg>',
                color: '#ea580c'
            },
            'WARD': {
                icon: '<svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4 4a2 2 0 012-2h8a2 2 0 012 2v12a1 1 0 110 2h-3a1 1 0 01-1-1v-2a1 1 0 00-1-1H9a1 1 0 00-1 1v2a1 1 0 01-1 1H4a1 1 0 110-2V4zm3 1h2v2H7V5zm2 4H7v2h2V9zm2-4h2v2h-2V5zm2 4h-2v2h2V9z" clip-rule="evenodd" /></svg>',
                color: '#2563eb'
            },
            'OR': {
            ...
            fetch('{{ route('core2.bed-linen.floor-map-data') }}')
                ...
                .then(wards => {
                    ...
                    wards.forEach(w => {
                        ...
                        if (grouped[z]) grouped[z].push(w);
                        else grouped['WARD'].push(w);
                    });
                    ...
                    Object.keys(grouped).forEach(zk => {
                        html +=
                            `<div id="bp-zone-${zk}" class="bp-zone-panel" style="display:none; font-family:'Inter', sans-serif;">`;
                        ...
                        `;
                            ...
                            `;
                                ...
                                ward.rooms.forEach(room => {
                                    html +=
                                        `<div class="bp-room" style="background:#fff; border:1px solid #e2e8f0; border-radius:16px; padding:20px; box-shadow:0 1px 2px 0 rgba(0,0,0,0.05); margin-bottom:16px;">`;
                                    html += `
                                ...
                                `;
                                    ...
                                    room.beds.forEach(bed => {
                                        ...
                                        let safeStatus = Object.keys(statusConfig).find(
                                            k => k.toLowerCase() === rawStatus
                                            .toLowerCase()) || 'Available';
                                        let cfg = statusConfig[safeStatus];

                                        const isOccupied = safeStatus === 'Occupied';
                                        const firstName = bed.patient_name ? bed
                                            .patient_name.split(' ')[0] : null;
                                        const tooltip = isOccupied && bed.patient_name ?
                                            `${bed.patient_name}\n${bed.mrn ? 'MRN: '+bed.mrn : ''}` :
                                            safeStatus;
                    ...
                    if (tabsContainer.innerHTML !== document.createRange().createContextualFragment('').innerHTML) {
                        ...
                        tabsContainer.innerHTML =
                        ''; // This is a bit dirty but it ensures we re-bind the onClick properly in this simple script 
                ...
                .catch(err => {
                    if (isInitialLoad) {
                        container.innerHTML =
                            `<p style="text-align:center; color:#ef4444; font-weight:bold; padding:50px 0; font-size:16px;">Failed to load floor map data.</p>`;
                    }
                    console.error(err);
                });
        }
        ...
        function openUpdateModal(el) {
            ...
            saveBtn.className =
                "px-5 py-2.5 rounded-xl text-xs font-bold text-white bg-indigo-600 shadow-sm hover:bg-indigo-700 transition flex items-center gap-2";
            ...
            if (status === 'Occupied') {
                grid.innerHTML = `
                <div class="col-span-2 p-4 bg-red-50 text-red-700 rounded-xl border border-red-200">
                    <div class="flex gap-3 mb-4">
                        ...
                        <div class="text-sm">
                            ...
                            Manual status changes are restricted while a patient is admitted.
                        </div>
                    </div>
                    <a href="{{ route('core2.bed-linen.pending-admissions.index') }}" class="block w-full text-center bg-indigo-600 text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition shadow-sm">
                        Transfer Patient
                    </a>
                </div>
            `;
                ...
                saveBtn.className =
                    "px-5 py-2.5 rounded-xl text-xs font-bold text-white bg-slate-400 shadow-sm cursor-not-allowed flex items-center gap-2";
            }
        ...
        function saveBedStatus() {
            ...
            btn.disabled = true;
            btn.innerHTML =
                `<svg class="w-4 h-4 animate-spin inline" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg> Saving…`;
        ...
        function showFlash(message, type = 'success') {
            ...
            flash.className =
                `core2-flash mx-10 mt-6 bg-${color}-50 border border-${color}-200 text-${color}-800 rounded-2xl px-6 py-4 text-sm font-semibold flex items-center gap-3 fixed top-4 right-4 z-[10000] shadow-lg transition-opacity opacity-0`;
